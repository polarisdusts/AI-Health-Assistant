...
function generateMockMonthlyPlan(user) {
  ...
  const exerciseFocus = user.diet_preference === "high_protein" ? "力量训练为主" : "有氧运动为主";
  ...
  return {
    ...
    exercise_plan: {
      weekly_frequency: user.activity_level === "high" ? 5 : user.activity_level === "medium" ? 3 : 2,
      recommended_types: [
        exerciseFocus,
      ...
      intensity: user.activity_level === "low" ? "低强度起步，逐步适应" : "中高强度交替",
...
function generateMockDailySuggestion(user, recentActivities) {
  ...
  return {
    ...
    activityCount,
    totalCalories,
    recommendedToday: activityCount < 3 ? "快走/散步" : activityCount < 8 ? "跑步/骑行" : "HIIT训练",
  };
}
...
function generateMockMonthlyReport(activities, month) {
  ...
  const qualifiedDays = activities.reduce((count, a) => {
    ...
    const daySteps = dayActivities.reduce((s, act) => s + (act.steps_estimated || 0), 0);
    const dayCal = dayActivities.reduce((s, act) => s + (act.calories_burned || 0), 0);
  ...
  return {
   ...
   qualifiedDays,
    ...
    sleepSummary: "本周你的睡眠片段零碎，请注意合理安排作息，建议固定作息时间并减少睡前使用电子设备。",
    nextMonthSuggestion: {
