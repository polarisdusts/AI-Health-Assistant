      ...
      res.on("end", () => {
        try {
          ...
          if (parsed.error) {
            reject(
              new Error(
                `DeepSeek API 错误: ${parsed.error.message || JSON.stringify(parsed.error)}`,
              ),
            );
          ...
          reject(
            new Error(
              `解析响应失败: ${e.message}, 原始响应: ${data.substring(0, 200)}`,
            ),
          );
        }
      });
...
JSON 结构必须如下：
{
  ...
  "summary": "一段50字内的总结建议"
      ...
      if (!parsed.diet_plan.recommended_foods) {
        parsed.diet_plan.recommended_foods = [];
      }
      if (!parsed.diet_plan.avoid_foods) {
        parsed.diet_plan.avoid_foods = [];
      }
      if (!parsed.exercise_plan.recommended_types) {
        parsed.exercise_plan.recommended_types = [];
      }
      return parsed;
    } catch (err) {
...
async function generateDailySuggestion(user, recentActivities) {
  if (API_KEY) {
    try {
      ...
      const totalCalories =
        recentActivities?.reduce((s, a) => s + (a.calories_burned || 0), 0) ||
        0;
...
/**
 * 专门生成30天详细每日运动计划（供前端 PlanGenerator 使用）
 */
async function generateDetailedExercisePlan(
  user,
  weightContext,
  monthlySummary,
  selectedTypes,
...
用户信息：${ageGroup}，${activityLabel}，BMI ${(user.weight / ((user.height / 100) * (user.height / 100))).toFixed(1)}
选择的运动类型：${typeNames || "多种运动"}
...
只返回JSON数组，不要其他文字。`;
      ...
      const response = await callDeepSeek(
        [
          {
            role: "system",
            content: "你是一名专业健身教练，每次只返回JSON数据。",
          },
          { role: "user", content: prompt },
        ],
        0.5,
      ...
      throw new Error("返回天数不足28天，实际:" + (days ? days.length : 0));
    } catch (err) {
      console.warn("DeepSeek 详细运动计划失败，回退本地生成:", err.message);
    }
  }
  return null; // 返回 null 让前端自行 fallback
}
